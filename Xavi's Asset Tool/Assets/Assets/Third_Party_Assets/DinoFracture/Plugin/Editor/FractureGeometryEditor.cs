...
namespace DinoFracture.Editor
{
    ...
    public class FractureGeometryEditor : UnityEditor.Editor
    {
        ...
        static FractureGeometryEditor()
        {
            ...
            AddPropertyName(_sCommonPropertyNames, "VertexMergingPolicy");
        ...
        protected static void ConvertComponentTo<T>(FractureGeometry geom) where T : FractureGeometry
        {
            T newGeom = geom.gameObject.AddComponent<T>();
            newGeom.CopyFrom(geom);
            ...
            {
                // Keep the same index
                var allComps = geom.gameObject.GetComponents<Component>().ToList();
                int idx = allComps.IndexOf(geom);
                for (int i = allComps.Count - 1; i > idx; --i)
                {
                    ComponentUtility.MoveComponentUp(newGeom);
                }

                DestroyImmediate(geom);
                EditorUtility.SetDirty(newGeom.gameObject);
            }
        }
        ...
        protected virtual void OnSceneGUI()
        {
            ...
            if (_editingPlanes && _selectedSlicePlaneIdx >= 0)
            {
                ...
                for (int i = 0; i < _targetData.Targets.Length; i++)
                {
                    ...
                    if (changed)
                    {
                        var newPlane = FractureGeometry.CreateSlicePlane(pos, rot * Vector3.forward, plane.Rotation * Vector3.up, _targetData.Targets[i].transform);

                        plane.Position = newPlane.Position;
                        plane.Rotation = newPlane.Rotation;
                        plane.Scale = scale;

                        var slicePlaneProp = _targetData.SlicePlanes[_selectedSlicePlaneIdx].Property;

                        slicePlaneProp.FindPropertyRelative("Position").vector3Value = newPlane.Position;
                        slicePlaneProp.FindPropertyRelative("Rotation").quaternionValue = newPlane.Rotation;
        ...
        {
            var piecesParentProp = serializedObject.FindProperty("PiecesParent");
            if (!piecesParentProp.hasMultipleDifferentValues)
            {
        ...
        private void GetPlaneWorldSpaceTransform(ref SlicePlaneSerializable plane, UnityEngine.Object owner, out Matrix4x4 worldTrans)
        {
            ...
            var localScale = ((MonoBehaviour)owner).transform.localScale;
            var invLocalScale = new Vector3(1.0f / localScale.x, 1.0f / localScale.y, 1.0f / localScale.z);
            var trans2 = trans * Matrix4x4.Scale(invLocalScale);

            Vector3 p0 = plane.Position + plane.Rotation * Vector3.right;
            Vector3 p1 = plane.Position + plane.Rotation * Vector3.up;
            Vector3 p2 = plane.Position;

            p0 = trans2.MultiplyPoint(p0);
            p1 = trans2.MultiplyPoint(p1);
            p2 = trans2.MultiplyPoint(p2);

            Plane newPlane = new Plane(p0, p1, p2);

            Vector3 worldPos = trans.MultiplyPoint(plane.Position);

            Vector3 planeUp = trans.MultiplyVector(plane.Rotation * Vector3.up).normalized;
            Quaternion worldRot = Quaternion.LookRotation(newPlane.normal, planeUp);
